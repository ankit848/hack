...
def submit_form(_token, username, email, password, password_confirmation, country, areacode):
    url = 'https://loyality-one.site/user/register'
    data = {
        '_token': _token,
        'username': username,
...
def main():
    ...
    for _ in range(2):
        ...
        mobile = int(mobile_base) + random.randint(1000, 9999)
        username = f'hacker{generate_random_numbers()}'
        email = f'{username}p{generate_random_numbers()}@gmail.com'
        
        submit_form(_token, username, email, password, password_confirmation, country, areacode)
